...
import { BsCheckCircleFill, BsCoin, BsStopwatch, BsWatch } from "react-icons/bs"

const AdminUserInstallMents = () => {

    const { id } = useParams()



    // const { customers[id], UserInstallMentPay } = useSelector(state => state.user)
                        ...
                        customers[id].installment.map((item, i) => {
                            ...
                            return <>
                                <div className={`rounded-xl  block transition-all duration-500  my-4  bg-white `}>
                                    <div key={i} className={`p-4 block  ${item.status === "successed" ? "alert  alert-success" : item.status === "bounced" ? "bg-red-600" : "bg-white"} rounded-xl `}>
                                        {/* .....{itemDay} */}
                                        ...
                                        <div className={`flex justify-between items-center `}>
                                            ...
                                            <div className="block">
                                                {
                                                    item.status !== "successed" && item.status !== "bounced" && <>
...
export default AdminUserInstallMents